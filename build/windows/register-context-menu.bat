...
REM
REM Usage:
REM   register-context-menu.bat en           English menu (default)
REM   register-context-menu.bat zh           Chinese menu
REM   register-context-menu.bat bilingual    Bilingual menu (中文 / English)
REM
...
REM ---- ProgID ----
reg add "HKLM\Software\Classes\OpenBox.Archive" /ve /d "%ARCHIVE_DESC%" /f >nul
...
reg add "HKLM\Software\Classes\OpenBox.Archive\shell\extract" /ve /d "%EXTRACT_MENU%" /f >nul
reg add "HKLM\Software\Classes\OpenBox.Archive\shell\extract\command" /ve /d "\"%OPENBOX_EXE%\" -x \"%%1\"" /f >nul
reg add "HKLM\Software\Classes\OpenBox.Archive\shell\extractHere" /ve /d "%EXTRACT_HERE_MENU%" /f >nul
...
reg add "HKLM\Software\Classes\*\shell\OpenBoxCompress" /ve /d "%COMPRESS_MENU%" /f >nul
...
reg add "HKLM\Software\Classes\Directory\shell\OpenBoxCompress" /ve /d "%COMPRESS_MENU%" /f >nul
...
reg add "HKLM\Software\Classes\Directory\Background\shell\OpenBoxCompress" /ve /d "%COMPRESS_MENU%" /f >nul
...
reg add "HKLM\Software\Classes\Directory\shell\OpenBoxExtract" /ve /d "%EXTRACT_MENU%" /f >nul
