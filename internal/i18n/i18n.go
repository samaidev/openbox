...
var table = map[key][2]string{
        ...
        // LanguageToggle button shows the CURRENT language (matching the UI
        // below). Clicking it switches to the other language. This matches
        // the user's expectation that the toggle button's language is the
        // same as the rest of the UI, not the opposite.
        ...
        About:                {"About", "关于"},
        AboutText: {"OpenBox — open-source cross-platform archiver.\nLicensed under MIT.\nProject: https://github.com/samaidev/openbox\n\nDeveloped by SamAI Studio (https://samai.cc) — \nbuilding practical AI tools for everyone.",
                "OpenBox — 开源跨平台压缩工具\n基于 MIT 协议发布\n项目地址：https://github.com/samaidev/openbox\n\n由 SamAI Studio (https://samai.cc) 开发——为每个人打造实用的 AI 工具。"},
        Ok:                {"OK", "确定"},
        Cancel:            {"Cancel", "取消"},
        Yes:               {"Yes", "是"},
        No:                {"No", "否"},
        ...
        // WinRAR-style toolbar buttons (Extract tab)
        AddToArchive:        {"Add", "添加"},
        DeleteEntry:         {"Delete", "删除"},
        Find:                {"Find", "查找"},
        FindPlaceholder:     {"Type to filter…", "输入以筛选…"},
        FindNext:            {"Next", "下一个"},
        NoMatchFound:        {"No matching entries.", "没有匹配的项。"},
        ConfirmDeleteEntry:  {"Delete the selected entry from the archive?\nThis rewrites the archive and cannot be undone.", "从压缩包中删除选中项？\n此操作会重写压缩包且无法撤销。"},
        ErrDeleteNotSupported: {"Delete is only supported for .zip archives.", "仅 .zip 压缩包支持删除。"},
        ErrAddNotSupported:    {"Add is only supported for .zip archives.", "仅 .zip 压缩包支持添加。"},
        DeleteSuccess:        {"Deleted: {name}", "已删除：{name}"},
        AddSuccess:           {"Added {n} file(s) to the archive.", "已向压缩包添加 {n} 个文件。"},
        TypeFolder:           {"Folder", "文件夹"},
        TypeFile:             {"File", "文件"},
        TypeSymlink:          {"Symlink", "符号链接"},
        CloseFind:            {"Close", "关闭"},
        UpBtn:                {"Up", "上一级"},
        OpenEntry:            {"Open", "打开"},
}
